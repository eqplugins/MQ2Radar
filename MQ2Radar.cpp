...
#ifndef DInput__MouseWheelScrolled 
#define DInput__MouseWheelScrolled 0x51F8E0 // 05-07-07 
#endif

int vDInput__MouseWheelScrolled = DInput__MouseWheelScrolled;
...
VOID Update_INIFileName() { 
	if (InGame()) { 
		sprintf(INIFileName,"%s\\MQ2Radar_%s.ini",gszINIPath,GetCharInfo()->Name); 
	} else { 
		sprintf(INIFileName,"%s\\MQ2Radar.ini",gszINIPath); 
	} 
} 
...
{	
	...
	GetPrivateProfileString("MQ2Radar","CEverQuest__LeftClickedOnPlayer",itoa(vCEverQuest__LeftClickedOnPlayer,&szTemp2[0],16),&szTemp[0],sizeof(szTemp),INIFileName);
	vCEverQuest__LeftClickedOnPlayer = (int)strtol(&szTemp[0],NULL,16);
	sprintf(szTemp, "%X", vCEverQuest__LeftClickedOnPlayer); 
	WritePrivateProfileString("MQ2Radar","CEverQuest__LeftClickedOnPlayer",szTemp,INIFileName); 

	GetPrivateProfileString("MQ2Radar","DInput__MouseWheelScrolled",itoa(vDInput__MouseWheelScrolled,&szTemp2[0],16),&szTemp[0],sizeof(szTemp),INIFileName);
	vDInput__MouseWheelScrolled = (int)strtol(&szTemp[0],NULL,16);
	sprintf(szTemp, "%X", vDInput__MouseWheelScrolled); 
...
{ 
	...
	GetPrivateProfileString("MQ2Radar","CenterX","100",&szTemp[0],sizeof(szTemp),INIFileName);
	gCenter.x = atoi(&szTemp[0]);
	sprintf(szTemp, "%d", (int)gCenter.x); 
	WritePrivateProfileString("MQ2Radar","CenterX",szTemp,INIFileName); 

	GetPrivateProfileString("MQ2Radar","CenterY","100",&szTemp[0],sizeof(szTemp),INIFileName);
	gCenter.y = atoi(&szTemp[0]);
	sprintf(szTemp, "%d", (int)gCenter.y); 
	...
	sprintf(szTemp, "%f", fgViewDistance); 
	...
	sprintf(szTemp, "%f", fgScale); 
	WritePrivateProfileString("MQ2Radar","Scale",szTemp,INIFileName); 

	GetPrivateProfileString("MQ2Radar","Zoom","1",&szTemp[0],sizeof(szTemp),INIFileName);
	fgZoom = (float)atof(&szTemp[0]);
	sprintf(szTemp, "%f", fgZoom); 
	WritePrivateProfileString("MQ2Radar","Zoom",szTemp,INIFileName); 

	GetPrivateProfileString("MQ2Radar","ZDepth","20",&szTemp[0],sizeof(szTemp),INIFileName);
	fgZDepth = (float)atof(&szTemp[0]);
	sprintf(szTemp, "%f", fgZDepth); 
	...
	sprintf(szTemp, "%d", igSkipPulses);
	WritePrivateProfileString("MQ2Radar","Delay",szTemp,INIFileName); 

	GetPrivateProfileString("MQ2Radar","Names","0",&szTemp[0],sizeof(szTemp),INIFileName);
	bgDrawNames = atoi(&szTemp[0])!=0;
	sprintf(szTemp, "%d", bgDrawNames); 
	...
	sprintf(szTemp, "%d", bgDrawTargetLine); 
	WritePrivateProfileString("MQ2Radar","TargetLine",szTemp,INIFileName); 

	GetPrivateProfileString("MQ2Radar","ShowSize","0",&szTemp[0],sizeof(szTemp),INIFileName);
	bgShowSize = atoi(&szTemp[0])!=0;;
	sprintf(szTemp, "%d", bgShowSize); 
	WritePrivateProfileString("MQ2Radar","ShowSize",szTemp,INIFileName); 

	GetPrivateProfileStringA("MQ2Radar","SizeUnit","0",&szTemp[0],sizeof(szTemp),INIFileName);
	fgSizeUnit = atof(&szTemp[0]);
	sprintf(szTemp, "%f", fgSizeUnit); 
...
{ 
	...
	sprintf(szTemp, "%d", (int)bgDrawNames); 
	WritePrivateProfileString("MQ2Radar","Names",szTemp,INIFileName); 

	sprintf(szTemp, "%d", (int)bgDrawMap); 
	WritePrivateProfileString("MQ2Radar","Map",szTemp,INIFileName); 

	sprintf(szTemp, "%d", (int)bgDrawTargetLine); 
	WritePrivateProfileString("MQ2Radar","TargetLine",szTemp,INIFileName); 

	sprintf(szTemp, "%d", (int)bgShowSize); 
	WritePrivateProfileString("MQ2Radar","ShowSize",szTemp,INIFileName); 
...
void LoadMap(char* pZone) {
	...
	line tempLine;
	gMap.clear();
	sprintf(cFileName,"%s\\maps\\%s_1.txt",gszEQPath,pZone); 
...
void DrawItems(PSPAWNINFO pChar, float heading) {
	...
	{ 
		if (GetDistance(pChar->X, pChar->Y, pItem->X, pItem->Y)<(fgZoom*fgViewDistance) && abs(pItem->Z-pChar->Z)<fgZDepth) {
			...
			{
				GetFriendlyNameForGroundItem(pItem, name);
	...
	{ 
		...
		if (GetDistance(pChar, pSpawn)<(fgZoom*fgViewDistance) && abs(pSpawn->Z-pChar->Z)<fgZDepth) {
			...
			if (MouseOn((pSpawn->X-pChar->X)/fgZoom, (pSpawn->Y-pChar->Y)/fgZoom, heading))
				if (EQADDR_MOUSECLICK->LeftClick && pSpawn!=(PSPAWNINFO)pTarget) pTarget=(EQPlayer*)pSpawn;
			...
			// If we are drawing ourselves just draw a proper white sprite
				...
				DSAddSprite(igSurface,0, 0, 0xffffffff,sSpawn,180.0f-pChar->Heading*0.703125f,1);
			...
			if (bgDrawNames && (GetSpawnType(pSpawn)==NPC || GetSpawnType(pSpawn)==PC)) {
				...
				if (pSpawn!=pChar)
					DSAddText(igSurface,newCoord.x*fgScale+gCenter.x, newCoord.y*fgScale+gCenter.y-20, newCoord.x*fgScale+gCenter.x+300,newCoord.y*fgScale+gCenter.y, CleanupName(pSpawn->Name), 0x0ffffff00);
			} else {
				// If mouse is over a radar spawn show the name
				if (MouseOn((pSpawn->X-pChar->X)/fgZoom, (pSpawn->Y-pChar->Y)/fgZoom, heading)) {
					DSAddText(igSurface,EQADDR_MOUSE->X, EQADDR_MOUSE->Y-20, EQADDR_MOUSE->X+300,EQADDR_MOUSE->Y, CleanupName(pSpawn->Name),0x0ffffff00);
				}		
			}
		}
		pSpawn=pSpawn->pNext;
...
PLUGIN_API VOID InitializePlugin()
{
	...
	HMODULE hMQ2Radar = GetModuleHandle("MQ2Radar.dll");
	...
	if (InitializeDS()) {
		...
		if (atof(DSGetVersion())>=0.110) {
			...
			if (vCEverQuest__LeftClickedOnPlayer!=0)
				EzDetour(vCEverQuest__LeftClickedOnPlayer,&CEQClick_Detour::EQClick_Detour,&CEQClick_Detour::EQClick_Trampoline);
			if (vDInput__MouseWheelScrolled!=0)
				EzDetour(vDInput__MouseWheelScrolled,&MouseScroll_Detour,&MouseScroll_Trampoline);
		}
		else {
			WriteChatColor("MQ2Radar (July 5/2007) requires DirectSurface 0.110 or older",CONCOLOR_RED);
...
PLUGIN_API VOID ShutdownPlugin(VOID)
{
	...
	if (!bgPluginInactive) {
		...
		if (vDInput__MouseWheelScrolled!=0)		
			RemoveDetour(DInput__MouseWheelScrolled);
